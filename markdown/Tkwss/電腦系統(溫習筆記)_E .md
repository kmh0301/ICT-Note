...
- **Operation Cycle (IPO Cycle):** The correct sequence of computer operation is:
<br> <u>　　　　　　　　</u> → <u>　　　　　　　　</u> → <u>　　　　　　　　</u>.
...
- **Pointing Devices:** Used for interacting with the computer interface, performing operations such as pointing, clicking, dragging, etc..
  ...
  - **Trackball Advantage:** It excels over a mouse in that it requires 
  <br><u>　　　　　　　　　　　　　　</u>.
- **Other Input Devices:** Keyboard, motion sensing devices, microphone, scanner, webcam, racing wheel.
- **Devices with both <br><u>　　　　　　　　　　　　　　</u>.:** Touch screen. A touch screen determines the touch location based on **electrical impulses from the human body**. Another example is a **monitor with built-in speakers**, or **headphones** (if they include a microphone).

## 2. Output Devices

- **Function:** To **convert digital messages into messages <u>　　　　　　　　　　</u> by humans**.
- **Common Output Devices:** Monitor, Printer, Speaker, Projector, Headphones/Earphones.
- **OLED Monitor Advantages:** Compared to LCD monitors, they
<br>generally have a higher <u>　　　　　　　　　　　　　　</u>
<br>, higher <u>　　　　　　　　　　　　　　</u>, wider <u>　　　　　　　　</u>, and lower weight.
...
- **Components inside the System Unit include:** <u>　　　　　　　　</u>, <br><br><u>　　　　　　　　　　　</u> (CPU), <u>　　　　　　　　　　　</u> (GPU), Power Supply, Primary Storage (Main memory), Secondary Storage, and Cooling System.
...
| Component Name | Volatility  | Data Property  | Function/Usage  |
| :--- | :--- | :--- | :--- |
| **Random Access Memory (RAM)** | <u>　　　　　　　　</u> | Data **is lost when the computer shuts down** | **Stores data temporarily** for processing by the CPU |
| **Read-only Memory (ROM)** | <u>　　　　　　　　</u> | Data **retains its data when the computer shuts down** | Stores **firmware/a collection of instructions required to boot a computer** (BIOS/Firmware) |
...
-  <u>　　　　　　　　　　　</u>(ALU): Responsible for **arithmetic operations** (e.g., addition, subtraction, multiplication, division) and **logical operations** (e.g., comparison, conjunction/AND, disjunction/OR, inversion/NOT).
<br>
- <u>　　　　　　　　　　　</u>(CU): Responsible for **decoding instructions**, issuing control signals, controlling the data flow, and commanding the operation of the ALU. The Control Unit is **not responsible for storing data**.
<br>
- <u>　　　　　　　　　　　</u>: Located inside the CPU. Used to store the **data values and data addresses currently being processed**. Registers have the **fastest access speed**, but the smallest capacity. CPU registers are at the highest level of the storage hierarchy.

## 2. Factors Affecting CPU Processing Capacity

The processing capacity of the CPU depends on the following factors:

- <u>　　　　　　　　　　　</u>: A higher number of cores means faster speed for **multitasking**. Multi-core CPUs generally have higher computing power because the workload can be distributed among different cores.
- <u>　　　　　　　　　　　</u>: Refers to the **speed at which a central processor can perform operations**. It is usually measured in gigahertz (GHz). A processor with a higher clock rate usually performs better, but also has **higher energy consumption**.
- <u>　　　　　　　　　　　</u>: Refers to the length of data a CPU processes at a time (e.g., 32-bit or 64-bit). A **64-bit CPU is <br><br><u>　　　　　　　　　　　</u> twice as fast as a 32-bit CPU**.
...
- **Storage Capacity Ranking (from smallest to largest):** **B < KB < MB < GB < TB**.
  - Conversion relationship: 1 Kilobyte (KB) = <u>　　　　　　</u> Bytes (B); 1 Megabyte (MB) = <u>　　　　　　</u> KB; 1 Gigabyte (GB) = <u>　　　　　　</u> MB; 1 Terabyte (TB) = <u>　　　　　　</u> GB.
- **Number System:** Computers use the **Binary number system** (base 2) to process data.

## 2. Secondary Storage

Secondary storage is used to store data for **future use**. All forms of <br><br>secondary storage are <u>　　　　　　　　</u>.
...
- **Advantages:**
  - Users can access data through the <u>　　　　　　　　</u> **anytime, anywhere** using their devices.
  ...
  - The risk of data loss is low because service providers automatically <br><br><u>　　　　　　　　　　　　　</u>.
  - Users can easily share files over the Internet.
- **Disadvantages:**
  - **Reliance on Internet connection**; data cannot be retrieved when <br><br><u>　　　　　　　　　</u>.
  - Data security depends entirely on the service provider's management.
  - Access time <u>　　　　　　　　　　</u> may occur if the server is busy, as servers are often shared among multiple users.
...
- The **higher** the storage device is placed in the hierarchy:
  1.  The <u>　　　　　　　　　　　　　</u> the data access speed.
  2.  The <u>　　　　　　　　　　　　　</u> the storage capacity.
  3.  The <u>　　　　　　　　　　　　　</u> the cost per unit capacity.
...
- <u>　　　　　　　　　　　　　</u> (OS): Manages the operation of computer hardware and programs.
...
- <u>　　　　　　　　　　　　　</u>: Used to maintain the computer system and improve its efficiency.
...
- <u>　　　　　　　　　　　　　</u>: Classified as **System Software**. Responsible for **controlling the operation of specific hardware**, such as the mouse, keyboard, and speakers.
...
- <u>　　　　　　　　　　　　　</u>: Word processors (e.g., Microsoft Word), spreadsheet software (e.g., Microsoft Excel), presentation software (e.g., Microsoft PowerPoint).
<br>
- <u>　　　　　　　　　　　　　</u>: Web browsers (e.g., Google Chrome), email clients (e.g., Microsoft Outlook), instant messaging applications (e.g., WhatsApp).
<br>
- <u>　　　　　　　　　　　　　</u>: Image editing software (e.g., Adobe Photoshop), audio editing software (e.g., Audacity), video editing software (e.g., video rendering, which often relies on the **GPU**).

---
# Lesson File Management and Shortcuts
...
- **Classification Principles:** Files are generally classified by **File Type**, **Date/Time Sequence**, and **Nature of Activity**. Subfolders should be created to classify files more meticulously.
  - **File Naming:** File names should use **concise text**. **Avoid using special characters** when naming files.
- **File Viewing Methods (Windows File Explorer):** View as Large Icons, View as Extra Large Icons, View as List, View as Details. **"View as Table" is not a common viewing method**.
- **Common File Formats:** Document files such as **PDF**, DOC, DOCX. Image files such as GIF, BMP, PNG, JPEG. Video files such as AVI.
...
- **Common Shortcuts:**
  ...
  - **Paste:** CTRL + <u>　　　　　　　　　　　　　</u>.
  - **Cut:** CTRL + <u>　　　　　　　　　　　　　</u>.
  - **Undo step/Cancel last action:** CTRL + <u>　　　　　　　　　　　　　</u>.
  - **Search files/Find:** CTRL + <u>　　　　　　　　　　　　　</u>.
  - **Select All:** CTRL + <u>　　　　　　　　　　　　　</u>.
